...
- **Three tags pushed**: `latest`, `X.Y.Z` (version), and `X.Y.Z-openssl-A.B.C`
- **Attestations**: Images include SBOM (`--sbom=true`) and provenance (`--provenance=true`) data
- **Verification**: Check with `docker buildx imagetools inspect cmooreio/nginx:<tag> --format '{{.Manifest.Digest}}'`
