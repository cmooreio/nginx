...
This project includes comprehensive CI/CD pipelines via GitHub Actions:

### Automated Workflows

- **Build & Test**: Multi-platform builds (amd64 + arm64) with smoke tests
- **Security Scanning**: Trivy, Hadolint, Grype with GitHub Security integration
- **SBOM Generation**: Syft generates SPDX and CycloneDX formats
- **Image Signing**: Cosign keyless signing via Sigstore
- **Publishing**: Automatic push to Docker Hub and GitHub Container Registry (ghcr.io)
- **Dependency Updates**: Renovate + Dependabot keep dependencies current

### Available Images

**Docker Hub**: `cmooreio/nginx:latest`
**GitHub Container Registry**: `ghcr.io/<your-username>/nginx:latest`

### Setup Instructions

To enable GitHub Actions publishing for your fork:

1. Add Docker Hub credentials (optional):
   - `DOCKERHUB_USERNAME` - your Docker Hub username
   - `DOCKERHUB_TOKEN` - access token from hub.docker.com

2. Update image names in `.github/workflows/ci.yml` and `Makefile`

3. GitHub Container Registry (ghcr.io) works automatically with no setup required

For detailed setup instructions, see [.github/INTEGRATION_SETUP.md](.github/INTEGRATION_SETUP.md)
...
cosign verify ghcr.io/<your-username>/nginx:1.29.3 \
  --certificate-identity https://github.com/<your-username>/nginx/.github/workflows/ci.yml@refs/heads/master \
