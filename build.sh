...
# Secure build script for nginx Docker image
...
validate_version() {
    local version="$1"
    # Allow x.y.z or x.y format (for PCRE2)
    if [[ ! "$version" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
        log_error "Invalid version format: $version (expected x.y.z or x.y)"
        return 1
    fi
}
...
validate_commit_sha() {
    ...
    if [[ ! "$sha" =~ ^[a-f0-9]{7,40}$ ]]; then
        log_error "Invalid commit SHA: $sha"
        return 1
    fi
}
...
load_versions() {
    ...
    local required_vars=(
        VERSION SHA256
        PCRE2_VERSION PCRE2_SHA256
        ZLIB_COMMIT_SHA
        OPENSSL_VERSION OPENSSL_SHA256
        BROTLI_COMMIT_SHA
        HEADERS_MORE_COMMIT_SHA
        FANCYINDEX_COMMIT_SHA
        NJS_COMMIT_SHA
    )
    ...
    validate_version "$OPENSSL_VERSION" || exit 1
    validate_version "$PCRE2_VERSION" || exit 1
    validate_sha256 "$SHA256" || exit 1
    validate_sha256 "$PCRE2_SHA256" || exit 1
    validate_sha256 "$OPENSSL_SHA256" || exit 1
    validate_commit_sha "$ZLIB_COMMIT_SHA" || exit 1
    validate_commit_sha "$BROTLI_COMMIT_SHA" || exit 1
    validate_commit_sha "$HEADERS_MORE_COMMIT_SHA" || exit 1
    validate_commit_sha "$FANCYINDEX_COMMIT_SHA" || exit 1
...
display_config() {
    ...
    echo "  zlib:         Cloudflare @ ${ZLIB_COMMIT_SHA:0:8}"
...
Secure build script for nginx Docker images with supply chain security features.

Options:
  --push          Push images to registry after build
  --no-cache      Build without using cache
  --platform      Specify platforms (default: linux/amd64,linux/arm64)
...
build_image() {
    ...
    build_args+=(
        ...
        "--build-arg" "PCRE2_SHA256=$PCRE2_SHA256"
        "--build-arg" "ZLIB_COMMIT_SHA=$ZLIB_COMMIT_SHA"
        "--build-arg" "ZLIB_SHA256=${ZLIB_SHA256:-}"
        "--build-arg" "OPENSSL_VERSION=$OPENSSL_VERSION"
        "--build-arg" "OPENSSL_SHA256=$OPENSSL_SHA256"
        "--build-arg" "BROTLI_COMMIT_SHA=$BROTLI_COMMIT_SHA"
        "--build-arg" "HEADERS_MORE_COMMIT_SHA=$HEADERS_MORE_COMMIT_SHA"
        "--build-arg" "FANCYINDEX_COMMIT_SHA=$FANCYINDEX_COMMIT_SHA"
        "--build-arg" "NJS_COMMIT_SHA=$NJS_COMMIT_SHA"
...
scan_image() {
    ...
    if command -v trivy &> /dev/null; then
        trivy image --severity HIGH,CRITICAL "cmooreio/nginx:$VERSION" || log_warn "Trivy scan found issues"
    elif command -v grype &> /dev/null; then
        grype "cmooreio/nginx:$VERSION" || log_warn "Grype scan found issues"
    else
...
sign_image() {
    ...
    cosign sign "cmooreio/nginx:$VERSION" || log_error "Failed to sign image"
}
...
display_success() {
    ...
    echo ""
    echo "Tags created:"
    echo "  - cmooreio/nginx:latest"
    echo "  - cmooreio/nginx:$VERSION"
    echo "  - cmooreio/nginx:$VERSION-openssl-$OPENSSL_VERSION"

    if [[ "$PUSH" != "true" ]]; then
        echo ""
        echo "To test locally:"
        echo "  docker run --rm -it -p 80:80 cmooreio/nginx:latest"
    ...
    if [[ "$SIGN_IMAGE" == "true" ]] && [[ "$PUSH" == "true" ]]; then
        ...
        echo "  cosign verify cmooreio/nginx:$VERSION"
    fi
}
